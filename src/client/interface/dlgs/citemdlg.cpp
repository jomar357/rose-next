...
unsigned
CItemDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CItemDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
...
CItemDlg::GetEquipSlot(POINT pt) {
    ...
    for (int iSlot = 0; iSlot < MAX_EQUIP_IDX - 1; ++iSlot) {
        SetRect(&rt,
            c_ptEquipedSlotOffsets[iSlot].x,
            c_ptEquipedSlotOffsets[iSlot].y,
            c_ptEquipedSlotOffsets[iSlot].x + 40,
            c_ptEquipedSlotOffsets[iSlot].y + 40);
...
void
CItemDlg::SwitchIcon(int iReal, int iVirtual) {
    if (iReal == 0)
        return;
...
//*-----------------------------------------------------------------------
CSlot*
CItemDlg::GetInvenSlotByRealIndex(int iIndex) {
    CIcon* pIcon = NULL;
...
void
CItemDlg::ApplySavedVirtualInventory(std::list<S_InventoryData>& list) {
    std::list<S_InventoryData>::iterator iter;
    for (iter = list.begin(); iter != list.end(); ++iter)
        SwitchIcon(iter->lRealIndex, iter->lVirtualIndex);
}
void
CItemDlg::GetVirtualInventory(std::list<S_InventoryData>& list) {
    list.clear();
...
unsigned
CItemDlg::ActionPerformed(CActionEvent* e) {
    assert(e);
    if (e == NULL)
        return 0;
    switch (e->GetID()) {
...
bool
CItemDlg::IsAvailableRepair(CIcon* pIcon) {
    assert(pIcon);
    if (pIcon == NULL)
        return false;
...
void
CItemDlg::AddActionEventListener2Slots() {
    int iIndex = 0;
