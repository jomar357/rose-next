...
public:
    ...
    enum { MAX_PENDING_REQUESTS = 32 };

    static CMapFilePrefetcher& Get() {
        static CMapFilePrefetcher instance;
        return instance;
    }

    void Request(const char* szMapFileBase) {
        if (!szMapFileBase || !*szMapFileBase)
            return;
        if (!IsEnabled())
            return;
        EnsureStarted();

        std::string key(szMapFileBase);
        EnterCriticalSection(&m_cs);
        if (m_Queue.size() < MAX_PENDING_REQUESTS && m_Seen.insert(key).second) {
            QueueItem item;
            item.base_path = key;
            item.generation = m_Generation;
            m_Queue.push_back(item);
            if (m_Seen.size() > 512) {
                m_Seen.clear(); // bound memory over long sessions
            }
        }
    ...
    unsigned int GetQueueDepth() {
        ...
        EnterCriticalSection(&m_cs);
        const unsigned int queue_depth = (unsigned int)m_Queue.size();
        LeaveCriticalSection(&m_cs);
        return queue_depth;
    }
...
private:
    ...
    static unsigned __stdcall WorkerEntry(void* p) {
        ((CMapFilePrefetcher*)p)->Run();
        return 0;
    }
    ...
    bool ShouldProcessGeneration(unsigned long generation) {
        EnterCriticalSection(&m_cs);
        const bool should_process = !m_bStop && m_bEnabled && generation == m_Generation;
        LeaveCriticalSection(&m_cs);
...
CQuadPatchManager::ViewCullingFunc(aabbBox* box) {
    /// During the brief zone-load window (gamestatemoveplanet sets this
    /// true then clears it) every patch must be reported inside the frustum
    /// so AddObject runs for all fixed objects. Outside that window we must
    /// do the real test — the original codebase had this guard commented
    /// out, which short-circuited culling globally: every patch in the 3x3
    /// map set was marked visible every frame, pinning resPatch at 2304
    /// and crushing FPS in dense outdoor zones.
...
unsigned int CMAP_PATCH::s_nInsertThisFrame = 0;
unsigned int CMAP_PATCH::s_nRemoveThisFrame = 0;
...
CTERRAIN::FreeZONE() {
    ...
    ZeroMemory(m_pMapBUFFER, sizeof(m_pMapBUFFER));
...
void
CTERRAIN::SetMapPrefetchEnabled(bool enabled) {
    const bool was_enabled = this->IsMapPrefetchEnabled();
...
bool
CTERRAIN::IsMapPrefetchEnabled() const {
    return CMapFilePrefetcher::Get().IsEnabled();
}

unsigned int
CTERRAIN::GetQueuedMapLoadCount() const {
    return (unsigned int)m_LoadOneMapData.size();
}

unsigned int
CTERRAIN::GetDirtyMapCount() const {
    return (unsigned int)m_DirtyMapList.size();
}
...
unsigned int
CTERRAIN::GetPendingUnloadCount() const {
    return (unsigned int)m_PendingUnloadList.size();
}

unsigned int
CTERRAIN::GetProximityColdCount() const {
    return m_PatchManager.GetColdProximityCount(m_nLastMappingX, m_nLastMappingY);
}

void
CTERRAIN::ResetStreamingState() {
    while (!m_LoadOneMapData.empty()) {
        tagLOAD_ONE_MAP_DATA* pOneMapData = m_LoadOneMapData.front();
        m_LoadOneMapData.pop_front();
        delete pOneMapData;
    }

    m_DirtyMapList.clear();
    m_PendingUnloadList.clear();
    ZeroMemory(m_pMAPS, sizeof(m_pMAPS));
...
//-------------------------------------------------------------------------------------------------
/// Map-unload hysteresis.
///
/// Problem: boundary-crossing triggers an immediate SubMAP, which tears down
/// terrain data synchronously. Curving back re-queues the just-torn-down map
/// from disk → stutter.
///
/// Fix: when SetCenterPosition would call SubMAP(wUpdateFLAG), it instead
/// calls DeferSubMAP(wUpdateFLAG), which records the doomed cells into
/// m_PendingUnloadList with the current tick. Each frame, DrainPendingUnloads
/// processes entries whose grace window expired or whose cells are now too
/// far away to be worth keeping. AddMAP cancels pending entries for cells
/// about to be re-added, so a curve-back inside the grace window is free.
//-------------------------------------------------------------------------------------------------

/// Grace window (ms) an unloaded map stays fully loaded in case the player
/// curves back. 3 s covers typical combat footwork and follower pathing
/// without bloating RAM for long.
static const DWORD kMapUnloadGraceMs = 3000;

/// Chebyshev-distance ring outside which a pending-unload map is considered
/// too far to revive (in zone-map units). Beyond this, we drop it early
/// regardless of the grace window.
static const int kMapUnloadHysteresisRadius = 2;

/// Minimum real time between queued map-loads drained on the main thread.
/// 150 ms spreads a bursty backlog into an invisible drip.
static const DWORD kMinMapLoadIntervalMs = 150;

/// Minimum real time between dirty-map cleanups on the main thread.
static const DWORD kMinDirtyFreeIntervalMs = 150;

void
CTERRAIN::DeferSubMAP(WORD wUpdateFLAG) {
    DWORD dwNow = GetTickCount();

    for (short nI = 0; s_SubSecIdx[wUpdateFLAG][nI] >= 0; nI++) {
        short nZoneMapXIDX = m_nCenterMapXIDX + s_SecAdjPos[s_SubSecIdx[wUpdateFLAG][nI]].m_nX;
        short nZoneMapYIDX = m_nCenterMapYIDX + s_SecAdjPos[s_SubSecIdx[wUpdateFLAG][nI]].m_nY;

        if (nZoneMapXIDX < 0 || nZoneMapXIDX >= MAP_COUNT_PER_ZONE_AXIS || nZoneMapYIDX < 0
            || nZoneMapYIDX >= MAP_COUNT_PER_ZONE_AXIS) {
            continue;
        }
        if (!m_pMAPS[nZoneMapYIDX][nZoneMapXIDX]) {
            continue;
        }

        /// Already queued for unload? Refresh its tick (player re-crossed the
        /// same boundary pair); don't stack duplicates.
        bool bAlreadyQueued = false;
        for (auto& pending: m_PendingUnloadList) {
            if (pending.m_nZoneMapX == nZoneMapXIDX && pending.m_nZoneMapY == nZoneMapYIDX) {
                pending.m_dwQueuedTick = dwNow;
                bAlreadyQueued = true;
                break;
            }
        }
        ...
        tagPENDING_UNLOAD entry;
        entry.m_nZoneMapX = nZoneMapXIDX;
        entry.m_nZoneMapY = nZoneMapYIDX;
        entry.m_dwQueuedTick = dwNow;
        m_PendingUnloadList.push_back(entry);

        /// Purge any still-queued disk loads for this cell: they are now doomed.
        /// Matches SubMAP()'s original behaviour.
        for (std::list<tagLOAD_ONE_MAP_DATA*>::iterator it = m_LoadOneMapData.begin();
             it != m_LoadOneMapData.end();) {
            tagLOAD_ONE_MAP_DATA* pOne = *it;
            short ex = pOne->m_nCenterX + s_SecAdjPos[pOne->m_nUpdateIndex].m_nX;
            short ey = pOne->m_nCenterY + s_SecAdjPos[pOne->m_nUpdateIndex].m_nY;
            if (ex == nZoneMapXIDX && ey == nZoneMapYIDX) {
                delete pOne;
                it = m_LoadOneMapData.erase(it);
            } else {
                ++it;
            }
        }
...
CTERRAIN::DrainPendingUnloads(DWORD dwNow) {
    ...
    for (std::list<tagPENDING_UNLOAD>::iterator it = m_PendingUnloadList.begin();
        ...
        if (dx > kMapUnloadHysteresisRadius || dy > kMapUnloadHysteresisRadius) {
            bShouldUnload = true;
        } else if ((DWORD)(dwNow - it->m_dwQueuedTick) >= kMapUnloadGraceMs) {
            bShouldUnload = true;
        }
...
void
CTERRAIN::DoActualUnload(short nZoneMapX, short nZoneMapY) {
    if (nZoneMapX < 0 || nZoneMapX >= MAP_COUNT_PER_ZONE_AXIS || nZoneMapY < 0
        || nZoneMapY >= MAP_COUNT_PER_ZONE_AXIS) {
        return;
    }

    CMAP* pMAP = m_pMAPS[nZoneMapY][nZoneMapX];
    if (!pMAP) {
        return;
    }
    ...
    m_pMAPS[nZoneMapY][nZoneMapX] = NULL;

    m_bPatchIndexDirty = true;
}

bool
CTERRAIN::CancelPendingUnload(short nZoneMapX, short nZoneMapY) {
    for (std::list<tagPENDING_UNLOAD>::iterator it = m_PendingUnloadList.begin();
         it != m_PendingUnloadList.end();
         ++it) {
        if (it->m_nZoneMapX == nZoneMapX && it->m_nZoneMapY == nZoneMapY) {
            LogString(LOG_DEBUG_,
                "		CancelPendingUnload: ( %d, %d )\n",
                nZoneMapX,
                nZoneMapY);
            m_PendingUnloadList.erase(it);
            return true;
...
CTERRAIN::Pick_OBJECT(D3DXVECTOR3& PosPICK, float& fDistance) {
    ...
    while (pNODE) {
        ...
        //박지호 :: 카트는 피킹을 하지않는다.
        // PVP QoL: a mounted player's big vehicle mesh (cart / castle gear) is a
        // separate object from the rider. Instead of skipping it, ray-test the
        // vehicle mesh and resolve a hit to the RIDER's object index so clicking
        // anywhere on the vehicle focuses the player riding it. The local player's
        // own vehicle (rider == OBJ_USER) stays unpickable, matching the avatar rule.
        if (pChar->Get_TYPE() == OBJ_CART || pChar->Get_TYPE() == OBJ_CGEAR) {
            CObjCHAR* pRider = ((CObjCART*)pChar)->GetParent();
            if (pRider && pRider->Get_TYPE() != OBJ_USER) {
                if (pChar->IsInViewFrustum() && pChar->IsIntersect(fCurDis)) {
                    if (fCurDis < fDistance) {
                        PosPICK = pChar->Get_PickPOSITION();
                        fDistance = fCurDis;
                        iPickObject = pRider->Get_INDEX();
                    }
                }
            }
            pNODE = g_pObjMGR->m_CharLIST.GetNextNode(pNODE);
...
CTERRAIN::AddMAP(short nCenterMapXIDX,
    ...
    if (!bImmediateLoad) {
        ...
        for (nI = 0; s_AddSecIdx[wUpdateFLAG][nI] >= 0; nI++) {
            /// Curve-back fast path: if this cell is pending unload, the
            /// terrain is still fully loaded. Cancel the pending unload and
            /// skip the disk-load enqueue entirely.
            short nCancelX =
                nCenterMapXIDX + s_SecAdjPos[s_AddSecIdx[wUpdateFLAG][nI]].m_nX;
            short nCancelY =
                nCenterMapYIDX + s_SecAdjPos[s_AddSecIdx[wUpdateFLAG][nI]].m_nY;
            if (nCancelX >= 0 && nCancelX < MAP_COUNT_PER_ZONE_AXIS && nCancelY >= 0
                && nCancelY < MAP_COUNT_PER_ZONE_AXIS
                && m_pMAPS[nCancelY][nCancelX]
                && CancelPendingUnload(nCancelX, nCancelY)) {
                continue;
            }
            ...
            if (pOneMapData) {
                ...
                /// Kick off a background prefetch so the file is warm in OS cache
                /// by the time the throttled main-thread load actually runs.
                if (FindLoadedMAP(nZoneX, nZoneY) == NULL) {
                    char* szPrefetchBase = GetMapFILE(nZoneX, nZoneY);
                    if (szPrefetchBase)
                        CMapFilePrefetcher::Get().Request(szPrefetchBase);
                }
            }
        }
    } else
...
CTERRAIN::SetCenterPosition(float fWorldX, float fWorldY) {
    /// Reset per-frame scene-toggle counters. The HUD (Render_GameMENU) runs
    /// after SetCenterPosition within the same frame, so it observes the
    /// counts accumulated during this call's Update_VisiblePatchManager.
    CMAP_PATCH::s_nInsertThisFrame = 0;
    CMAP_PATCH::s_nRemoveThisFrame = 0;
    CPatchManager::s_nRingStampsThisFrame = 0;
    ...
    //----------------------------------------------------------------------------------------------------
    if (!m_LoadOneMapData.empty()
        && (DWORD)(dwNow - m_dwLastMapLoadTick) >= kMinMapLoadIntervalMs) {
        tagLOAD_ONE_MAP_DATA* pOneMapData = NULL;
    ...
    if (true) {
        ...
        /// m_ppPATCH[48x48] holds patch pointers that become stale the
        ...
        m_PatchManager.Update_VisiblePatchManager(nMappingX, nMappingY);
        m_nLastMappingX = nMappingX;
        m_nLastMappingY = nMappingY;
    }
