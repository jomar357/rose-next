...
CFireSkill::Action(CObjCHAR* pAttacker, CObjCHAR* pTarget) {
    ...
    if (pTarget && iBulletIDX) {
        if (!g_pBltMGR->Add_BULLET(pAttacker, pTarget, iBulletIDX)) {
            LogString(LOG_DEBUG_,
                "CombatTrace projectile creation failed: caster %d target %d skill %d bullet %d\n",
                pAttacker ? pAttacker->Get_INDEX() : 0,
                pTarget->Get_INDEX(),
                GetSkillIndex(),
                iBulletIDX);
            pTarget->DiscardQueuedCombatDamageFromAttacker(pAttacker);
        }
        ...
        case SKILL_ACTION_FIRE_BULLET: {
            ...
            if (pTarget && iBulletIDX) {
                ...
                if (pBullet) {
                    ...
                    LogString(LOG_DEBUG_,
                        "CombatTrace projectile creation failed: caster %d target %d skill %d bullet %d\n",
                        pSrc ? pSrc->Get_INDEX() : 0,
                        pTarget->Get_INDEX(),
                        nSkillIdx,
                        iBulletIDX);
                    pTarget->DiscardQueuedCombatDamageFromAttacker(pSrc);
                }
