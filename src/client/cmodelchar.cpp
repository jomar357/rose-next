...
//-------------------------------------------------------------------------------------------------
CEffect**
CCharMODEL::CreateBoneEFFECT(HNODE hMODEL, CObjCHAR* pOwner) {
    //	return NULL;
    if (m_nBoneEFFECT < 1)
        return NULL;
        ...
        if (ppEffect[nE]) {
            ...
            if (iDummyCnt >= m_pBoneEFFECT[nE].m_cBoneIDX) {
                ...
                CBoneEffectBudget::Instance().Register(
                    pOwner,
                    ppEffect[nE],
                    m_pBoneEFFECT[nE].m_EffectFILE,
                    m_pBoneEFFECT[nE].m_cBoneIDX);
                ppEffect[nE]->SetParticleBatchRenderHint(true);
                /*D3DVECTOR pos;
...
CCharMODEL::DeleteBoneEFFECT(CEffect**& ppEffect) {
    if (ppEffect) {
        for (short nE = 0; nE < m_nBoneEFFECT; nE++) {
            ...
            if (ppEffect[nE]) {
                CBoneEffectBudget::Instance().Unregister(ppEffect[nE]);
