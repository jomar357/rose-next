    ...
    switch (iResult) {
        ...
        case LUA_ERRERR:
            LogString(LOG_NORMAL,
                "Script File ERROR[ %d ] %s \n",
                iResult,
                "Scripts\\GlobalSCR.lua");
            SAFE_DELETE(m_pLUA);
            return;
    }
}
...
int
GF_GetEffectUseIndex(int iEffectIndex) {
...
//-------------------------------------------------------------------------------------------------
int
GF_GetMotionUseFile(ZSTRING szMotionFile) {
...
GF_disorganizeClan(int iNPCNO) {
    if (CClan::GetInstance().GetClass() == CClan::CLAN_MASTER) {
        if (CClan::GetInstance().GetMemberCount() >= 2) {
            ...
            g_itMGR.OpenMsgBox(STR_CLAN_QUERY_DISORGANIZE,
...
//-------------------------------------------------------------------------------------------------
void
scrTest(int i, int j, char* szStr) {
    lua_CallIntFUNC(g_pScript->m_pLUA->m_pState,
        "test",
        ZZ_PARAM_INT,
        i,
        ZZ_PARAM_INT,
...
int
Call_Char_Lua_Func(int iObject, const char* pFunc) {
    if (!g_pScript->m_pLUA)
        return 0;

    int iResult =
        lua_CallIntFUNC(g_pScript->m_pLUA->m_pState, pFunc, ZZ_PARAM_INT, iObject, ZZ_PARAM_END);
...
///
void
GF_LogString(ZSTRING szMessage) {
...
SE_WeatherEffect(int iObject, int iWeatherType) {
    ...
    int iResult = lua_CallIntFUNC(g_pScript->m_pLUA->m_pState,
        "SE_WeatherEffect",
        ZZ_PARAM_INT,
        iObject,
        ZZ_PARAM_INT,
