...
CObjCHAR::ActionBow(int iActionIDX) {
    ...
    if (pTarget) {
        switch (iActionIDX) {
                /// 발사, 사운드
            case 22: {
                // Mounted rider attack motions are visual-only. The cart / castle
                // gear child owns projectile timing and damage consumption.
                if (this->GetPetMode() >= 0 && !this->IsPET()) {
                    break;
                }
...
CObjCHAR::FireEffectBullet(CObjCHAR* pTarget, int iBulletIDX, bool bDummy, int iSkillIDX) {
    if (pTarget && iBulletIDX) {
        ...
        if (!bDummy) {
            pBullet = g_pBltMGR->Add_BULLET(this,
                pTarget,
                iBulletIDX,
                (iSkillIDX == 0),
                0,
