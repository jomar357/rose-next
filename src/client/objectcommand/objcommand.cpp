...
CObjCommand*
CObjCommandManager::GetObjCommand(int iCommandType) {
    switch (iCommandType) {
...
int
CObjCommandManager::GetCommandCount() {
...
//----------------------------------------------------------------------------------------------------
/// @param
/// @brief 가장 최근에 들어온 스킬 명령을 찾는다. 왜냐.. Skill start 를 받았을 경우에는 가장 최신에
/// 받은
///		   스킬명령의 것이다.
//----------------------------------------------------------------------------------------------------
...
CObjCommandManager::GetLastInvalidSkillCommand() {
    ...
    bool bClearDummyCmd = false; /// 중간에 취소되야할 명령들( start_of_skill 이 왔을때 이전에
                                 /// 명령만있는것들은 모두 제거 )
...
void
CObjCommandManager::SetValidFlag() {
...
///			큐에 들어간 스킬명령은 결과를 받은후 그 플래그가 하나밖에 없어 이전 스킬이 이걸
///리셋할수있다 그래서 저장..
//----------------------------------------------------------------------------------------------------

void
CObjCommandManager::SetValidResultOfSkill() {
    CObjCommand* pObjCommand = NULL;
...
CObjCommand*
CObjCommandManager::PopCommand(bool& bSkillCommand) {
    if (IsEmpty())
        return NULL;
...
void
CObjCommandManager::ClearCommand() {
    std::list<CObjCommand*>::iterator begin = m_ObjCommandList.begin();
...
bool
CObjStopCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
...
void
CObjAttackCommand::SetCMD_ATTACK(int iServerIndex, WORD wSrvDIST, const D3DVECTOR& PosGOTO) {
...
bool
CObjAttackCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
bool
CObjSitCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
bool
CObjStandCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
void
CObjMoveCommand::SetCMD_MOVE(const D3DVECTOR& PosTO, BYTE btRunMODE) {
    m_bMoveToObjectMode = false;
...
void
CObjMoveCommand::SetCMD_MOVE(WORD wSrvDIST, const D3DVECTOR& PosTO, int iServerTarget) {
...
bool
CObjMoveCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
bool
CObjDieCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
void
CObjToggleCommand::SetCMD_TOGGLE(BYTE btTYPE) {}
...
bool
CObjToggleCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
void
CObjSkill2SelfCommand::SetCMD_Skill2SELF(short nSkillIDX) {
...
bool
CObjSkill2SelfCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
void
CObjSkill2ObjCommand::SetCMD_Skill2OBJ(WORD wSrvDIST,
...
void
CObjSkill2ObjCommand::SetCMD_Skill2OBJ(int iServerTarget, short nSkillIDX) {
    m_iSkillIDX = nSkillIDX;
    m_iServerTarget = iServerTarget;
}
...
bool
CObjSkill2ObjCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
...
void
CObjSkill2PosCommand::SetCMD_Skill2POS(const D3DVECTOR& PosGOTO, short nSkillIDX) {
    m_PosGOTO = PosGOTO;
    m_iSkillIDX = nSkillIDX;
}
...
bool
CObjSkill2PosCommand::Execute(CObjCHAR* pObjCHAR) {
    if (pObjCHAR == NULL) {
        assert(0 && "Command::Execute parameter is NULL:");
        return false;
    }
