...
CAI_OBJ*
CObjTARGET::Get_TargetOBJ(bool bCheckHP) {
    if (m_iServerTarget) {
...
CObjAI::Start_ATTACK(CObjCHAR* pTarget) {
    ...
    {
        // 공격 애니메이션 & 속도 설정. :: 크리티컬 공격일 경우 모션을 별도로 존재하지 않고 효과로
        // 출력한다.
...
CObjAI::Start_MOVE(float fSpeed) {
    ...
    m_iMoveDistance =
        distance((int)m_PosCUR.x, (int)m_PosCUR.y, (int)m_PosGOTO.x, (int)m_PosGOTO.y);
    this->Reset_MoveVEC();
...
/// Move command 세팅.
bool
CObjAI::SetCMD_MOVE(tPOINTF& PosFROM, tPOINTF& PosTO, int iServerTarget) {
#ifdef __BLOCK_WHEN_SKILL
    if (m_nActiveSkillIDX) {
        return false;
    }
...
bool
CObjAI::SetCMD_Skill2OBJ(int iServerTarget, short nSkillIDX) {
#ifdef __BLOCK_WHEN_SKILL
    if (m_nActiveSkillIDX) {
        return false;
    }
#endif
    if (nSkillIDX == m_nToDoSkillIDX && CMD_SKILL2OBJ == m_wCommand
        && iServerTarget == this->Get_TargetIDX())
        return false;
...
bool
CObjAI::SetCMD_Skill2POS(tPOINTF& PosGOTO, short nSkillIDX) {
#ifdef __BLOCK_WHEN_SKILL
    if (m_nActiveSkillIDX) {
        return false;
    }
...
CObjAI::Goto_POSITION(int iRange) {
    ...
    int iDistance =
        distance((int)m_PosCUR.x, (int)m_PosCUR.y, (int)m_PosMoveSTART.x, (int)m_PosMoveSTART.y);
