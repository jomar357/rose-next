...
CUserDATA::Cal_AvoidRATE() {
    if (this->GetCur_MOVE_MODE() > MOVE_MODE_RUN) {
        // Riding/driving: simple formula, no armor contribution
        m_Battle.m_nAVOID = (short)((GetCur_DEX() + 10) * 0.8f + GetCur_LEVEL() * 0.5f);
    } else {
        // Walk/run: AVO = (DEX*1.9 + LV*0.3 + 10)*0.4 + DefDura*0.3 + DefGrade
...
//-------------------------------------------------------------------------------------------------
bool
CUserDATA::Check_PatEquipCondition(tagITEM& sITEM, short nEquipIdx) {
...
bool
CUserDATA::Check_EquipCondition(tagITEM& sITEM, short nEquipIdx) {
    ...
    if (ITEM_TYPE_JEWEL == sITEM.GetTYPE()) {
        ...
        switch (ITEM_TYPE(sITEM.GetTYPE(), sITEM.GetItemNO())) {
            case JEWEL_ITEM_RING:
                if (nEquipIdx != EQUIP_IDX_RING)
                    return false;
                break;
            case JEWEL_ITEM_NECKLACE:
                if (nEquipIdx != EQUIP_IDX_NECKLACE)
                    return false;
                break;
            case JEWEL_ITEM_EARRING:
                if (nEquipIdx != EQUIP_IDX_EARRING)
                    return false;
                break;
        }
    }
