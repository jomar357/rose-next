...
*/
    ...
    switch (pAct->btTarget) {
        case 0:
            if (pEVENT->m_pFindCHAR) {
                CAI_OBJ* pCurrentTarget = pEVENT->m_pSourCHAR->Get_TARGET(false);
                const bool bTargetIsCurrentTarget =
                    pCurrentTarget && pCurrentTarget->Get_TAG() == pEVENT->m_pFindCHAR->Get_TAG();
                const bool bTargetIsAllied =
                    pEVENT->m_pSourCHAR->Is_ALLIED(pEVENT->m_pFindCHAR);

                if (Rose::ServerAI::should_block_non_aggro_script_skill(bTargetIsAllied,
                        bTargetIsCurrentTarget,
...
F_AIACT29(stActHead* pActDATA, t_AIPARAM* pEVENT) {
    ...
    // Desired trailing distance from owner (game units)
    const float FOLLOW_DISTANCE = 300.0f;

    float dx = pEVENT->m_pSourCHAR->Get_CurXPOS() - pOwner->Get_CurXPOS();
    float dy = pEVENT->m_pSourCHAR->Get_CurYPOS() - pOwner->Get_CurYPOS();
    float fDistance = sqrtf(dx * dx + dy * dy);

    if (fDistance < 1.0f)
        return; // Already on top of owner

    // Move to FOLLOW_DISTANCE units from owner in the summon's current direction
    float fPosX = pOwner->Get_CurXPOS() + (dx / fDistance) * FOLLOW_DISTANCE;
    float fPosY = pOwner->Get_CurYPOS() + (dy / fDistance) * FOLLOW_DISTANCE;
