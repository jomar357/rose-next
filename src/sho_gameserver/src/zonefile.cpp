...
*/
tPOINTF
CZoneFILE::Get_RevivePOS(tPOINTF PosCUR) {
...
//-------------------------------------------------------------------------------------------------
void
CZoneFILE::ReadObjINFO(FILE* fp, long lOffset, int iLumpType, short nMapXIDX, short nMapYIDX) {
    ...
    for (int iC = 0; iC < iObjCNT; iC++) {
        ...
        fPosX += (MAP_COUNT_PER_ZONE_AXIS / 2
        ...
        fPosY += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
        ...
        switch (iLumpType) {
            ...
            case LUMP_TERRAIN_MOB: {
                ...
                if (NPC_TYPE(iObjID) == 999) {
...
#ifdef __NPC_TEST
                    if (1013 != iObjID || this->Get_ZoneNO() != 99)
                        continue;
#endif
                    ...
                    if (!pNewCHAR->Init(this->GetZonePTR(),
                            iObjID,
                            fPosX,
                            fPosY,
                            TEAMNO_NPC,
...
//-------------------------------------------------------------------------------------------------
void
CZoneFILE::LoadMOV(char* szFileName, short nMapXIDX, short nMapYIDX) {
    // *.MOV
    char* szFullPath;
    FILE* fp;

    szFullPath =
        this->m_StrVAR.Printf("%s.MOV", szFileName); // CStr::Printf( "%s.IFO", szFileName );
...
//-------------------------------------------------------------------------------------------------
void
CZoneFILE::LoadMAP(char* szFileName, short nMapXIDX, short nMapYIDX) {
    char* szFullPath;
    FILE* fp;
    ...
    szFullPath =
        this->m_StrVAR.Printf("%s.IFO", szFileName); // CStr::Printf( "%s.IFO", szFileName );
...
//-------------------------------------------------------------------------------------------------
void
CZoneFILE::ReadZoneINFO(FILE* fp, long lOffset) {
    int iValue;
    float fValue;
    fseek(fp, lOffset, SEEK_SET);
    ...
    for (nY = 0; nY < m_iSectorYCnt; nY++) {
        ...
        for (nX = 0; nX < m_iSectorXCnt; nX++)
            m_ppSECTOR[nY][nX].SetCenterPOS(nX * m_nSectorSIZE + DIV02(m_nSectorSIZE),
                nY * m_nSectorSIZE + DIV02(m_nSectorSIZE));
    }
...
bool
CZoneFILE::ReadECONOMY(FILE* fp, long lOffset) {
    fseek(fp, lOffset, SEEK_SET);
...
//-------------------------------------------------------------------------------------------------
void
CZoneFILE::ReadEventObjINFO(FILE* fp, long lOffset) {
    int iEventPosCNT;
    ...
    */
    ...
    for (int iC = 0; iC < iEventPosCNT; iC++) {
        ...
        fX += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
        fY += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
...
//-------------------------------------------------------------------------------------------------
char*
CZoneFILE::GetMapFILE(short nMapX, short nMapY) {
