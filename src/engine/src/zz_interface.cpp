...
ZZ_SCRIPT
int setParticleRuntimeBudget ( HNODE hParticle, float fEmitScale, int iMaxParticles, int iUpdateIntervalMs )
{
	CHECK_INTERFACE(setParticleRuntimeBudget);
	zz_particle_emitter * part = reinterpret_cast<zz_particle_emitter*>(hParticle);

	if (!part) return 0;
	part->SetRuntimeBudget(fEmitScale, iMaxParticles, iUpdateIntervalMs);
	return 1;
}

ZZ_SCRIPT
int setParticleBatchHint ( HNODE hParticle, int iEnabled )
{
	CHECK_INTERFACE(setParticleBatchHint);
	zz_particle_emitter * part = reinterpret_cast<zz_particle_emitter*>(hParticle);

	if (!part) return 0;
	part->SetBatchRenderHint(iEnabled != 0);
	return 1;
}

ZZ_SCRIPT
int getParticleConfiguredCapacity ( HNODE hParticle )
{
	CHECK_INTERFACE(getParticleConfiguredCapacity);
	zz_particle_emitter * part = reinterpret_cast<zz_particle_emitter*>(hParticle);

	return part ? part->GetConfiguredParticleCapacity() : 0;
}

ZZ_SCRIPT
int getParticleRuntimeCapacity ( HNODE hParticle )
{
	CHECK_INTERFACE(getParticleRuntimeCapacity);
	zz_particle_emitter * part = reinterpret_cast<zz_particle_emitter*>(hParticle);

	return part ? part->GetRuntimeParticleCapacity() : 0;
}

ZZ_SCRIPT
int getParticleActiveCount ( HNODE hParticle )
{
	CHECK_INTERFACE(getParticleActiveCount);
	zz_particle_emitter * part = reinterpret_cast<zz_particle_emitter*>(hParticle);

	return part ? part->GetNumActiveParticles() : 0;
}

ZZ_SCRIPT
float getParticleConfiguredEmitRate ( HNODE hParticle )
{
	CHECK_INTERFACE(getParticleConfiguredEmitRate);
	zz_particle_emitter * part = reinterpret_cast<zz_particle_emitter*>(hParticle);

	return part ? part->GetConfiguredEmitRate() : 0.0f;
}

ZZ_SCRIPT
int getParticleBatchGroups ( void )
{
	CHECK_INTERFACE(getParticleBatchGroups);
	return zz_particle_emitter::GetBatchStats().groups;
}

ZZ_SCRIPT
int getParticleBatchParticles ( void )
{
	CHECK_INTERFACE(getParticleBatchParticles);
	return zz_particle_emitter::GetBatchStats().particles;
}

ZZ_SCRIPT
int getParticleBatchDrawCalls ( void )
{
	CHECK_INTERFACE(getParticleBatchDrawCalls);
	return zz_particle_emitter::GetBatchStats().draw_calls;
}

ZZ_SCRIPT
int getParticleBatchFallback ( void )
{
	CHECK_INTERFACE(getParticleBatchFallback);
	return zz_particle_emitter::GetBatchStats().fallback;
}

ZZ_SCRIPT
int getParticleBatchSavedDrawCalls ( void )
{
	CHECK_INTERFACE(getParticleBatchSavedDrawCalls);
...
ZZ_SCRIPT
int resetScreenIfNeeded ( ZSTRING reason )
{
	CHECK_INTERFACE(resetScreenIfNeeded);
	if (!znzin) {
		return 0;
	}
	...
	if (r->screen_state_matches() && !r->get_device_lost()) {
		ZZ_LOG("interface: resetScreenIfNeeded(%s) skipped.\n", reason ? reason : "unknown");
		return 0;
	}

	ZZ_LOG("interface: resetScreenIfNeeded(%s) resetting.\n", reason ? reason : "unknown");
	r->cleanup();
	r->initialize();
	return 1;
}
